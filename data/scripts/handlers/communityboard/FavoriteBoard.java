...
public class FavoriteBoard implements IParseBoardHandler
{
  ...
  private static final String SELECT_NEWS_AND_UPDATES = "SELECT * FROM `newsandupdates` ORDER BY `id` ASC";
  ...
  public boolean parseCommunityBoardCommand(String command, PlayerInstance player)
  {
    ...
    if (command.equals("_bbsgetSrv") || command.equals("_bbstop"))
    {
      CommunityBoardHandler.getInstance().addBypass(player, "Services", command);
      
      returnHtml = HtmCache.getInstance().getHtm(player, "data/html/CommunityBoard/services.html");
