...
public class HomeBoard implements IParseBoardHandler
{
  ...
  public boolean parseCommunityBoardCommand(String command, PlayerInstance player)
  {
    ...
    if (command.equals("_bbshome") || command.equals("_bbstop"))
    {
      ...
      String clanLeader = "None";
      String clanName = "None";
      String clanFort = "None";
      String clanCastle = "None";
      String clanHall = "None";
      ...
      if (hasClan)
      {
        ...
        if (clan.getFortId() > 0)
        {
          clanFort = FortManager.getInstance().getFortById(clan.getFortId()).getName();
        }
        if (clan.getCastleId() > 0)
        {
          clanCastle = CastleManager.getInstance().getCastleById(clan.getCastleId()).getName();
        }
        if (clan.getHideoutId() > 0)
        {
          clanHall = ClanHallManager.getInstance().getClanHallById(clan.getHideoutId()).getName();
        }
      }
      else
      {
      ...
      returnHtml = returnHtml.replace("%playerLevel%", String.valueOf(player.getLevel()));
      returnHtml = returnHtml.replace("%playerSubclass%", "1");
      returnHtml = returnHtml.replace("%playerNoblesse%", player.isNoble() ? "Yes" : "No");
      returnHtml = returnHtml.replace("%playerCoinsCount%", "23 Coins");
      returnHtml = returnHtml.replace("%playerPremiumLeft%", "3days 2hours");
      /* get player clan information */
      returnHtml = returnHtml.replace("%playerClanLeader%", clanLeader);
      returnHtml = returnHtml.replace("%playerClanName%", clanName);
      returnHtml = returnHtml.replace("%playerClanFort%", clanFort);
      returnHtml = returnHtml.replace("%playerClanCastle%", clanCastle);
      returnHtml = returnHtml.replace("%playerClanhall%", clanHall);

      playerCount = World.getInstance().getPlayers().size();
      returnHtml = returnHtml.replace("%onlinePlayersCount%",  String.valueOf(playerCount));
